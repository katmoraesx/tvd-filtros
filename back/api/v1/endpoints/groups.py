...
async def create_group(
    group: GroupSchemaCreate, db: AsyncSession = Depends(get_session)
):
    # Verifica se o nome já existe
    existing = await db.execute(select(GroupsModel).filter_by(name=group.name))
...
async def get_group(group_id: int, db: AsyncSession = Depends(get_session)):
    result = await db.execute(
        select(GroupsModel)
        .options(selectinload(GroupsModel.characters))
